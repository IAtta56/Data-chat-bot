...
try:
    ...
    # Fallback for langchain versions that don't expose AgentType
    class _AgentType:
...
if user_prompt:
    ...
    # loading the LLM - prefer Ollama if available, otherwise fall back to the project's Hugging Face LLM
    if ChatOllama is not None:
        llm = ChatOllama(model="gemma2:2b", temperature=0)
    else:
        try:
            from backend.agent_factory import get_llm
            llm = get_llm()
        except Exception:
            # final fallback: simple local dummy LLM that returns an error-friendly message
            class _DummyLLM:
